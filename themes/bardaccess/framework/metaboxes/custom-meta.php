					...
					switch( $type ) {
						...
						case 'select':
							echo '<select name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '"' , isset( $multiple ) && $multiple == true ? ' multiple="multiple"' : '' , '>
									<option value="">&mdash; ' . __('Select', 'bas') . ' &mdash;</option>'; // Select One
						...
						case 'post_list':
							...
							echo '</select> &nbsp;<span class="description"><a href="' . admin_url( 'edit.php?post_type=' . $post_type . '">' . __('Manage ', 'bas') . $post_type_object->label ) . '</a></span><br />' . $desc;
						break;
						// post_checkboxes
						...
						case 'post_drop_sort':
							...
							echo '<ul class="post_drop_sort_source sort_list">
									<li class="post_drop_sort_area_name">' . __('Available ', 'bas') . $label . '</li>';
							foreach ( $posts as $item ) {
						...
						case 'tax_checkboxes':
							...
							$tax = get_taxonomy( $taxonomy );
							echo '<span class="description">' . $desc . ' <a href="'.get_bloginfo( 'url' ) . '/wp-admin/edit-tags.php?taxonomy=' . $taxonomy . '&post_type=' . $page . '">' . __('Manage ', 'bas') . $tax->label . '</a></span>';
						break;
						// date
						case 'date':
						...
						// image
						case 'image':
							$image = get_template_directory_uri() . '/library/inc/metaboxes/images/image.png';	
						...
						case 'file':		
							...
							echo	'<div class="meta_box_file_stuff"><<input name="' . esc_attr( $name ) . '" type="hidden" class="meta_box_upload_file" value="' . esc_url( $meta ) . '" />
										...
										<span class="meta_box_filename">' . esc_url( $meta ) . '</span>
											<a href="#" class="meta_box_upload_image_button button" rel="' . get_the_ID() . '">' . __('Choose File', 'bas'). '</a>
											<small>&nbsp;<a href="#" class="meta_box_clear_file_button">' . __('Remove File', 'bas') . '</a></small></div>
											<br clear="all" />' . $desc;
						break;
						// repeatable
						case 'repeatable':
