...
class TitanFrameworkAdminTab {
	...
	function __construct( $settings, $owner ) {
		...
		if ( !empty( $this->settings['icon'] ) ) {
			$this->icon = '<i class="' . $this->settings['icon'] . ' fa-fw"></i>';
		} else {
			$this->icon = '';
		}

		if ( !empty( $this->settings['color'] ) ) {
			$this->color = 'background:' . $this->settings['color'] . '';
		} else {
			$this->color = '';
		}
	...
	public function displayTab() {
		...
		?>
		<li class="nav-tab carbon-menu <?php echo $this->isActiveTab() ? "nav-tab-active" : '' ?>" style="<?php echo $this->color; ?>">
			<a href="<?php echo esc_url( $url ) ?>" class="carbon-menu-link"><?php echo $this->icon ?> <?php echo $this->settings['name'] ?></a>
		</li>
		<?php
	}
